...
- [x] **#9** Fast-path bail in `parse_open_curly_bracket` after consuming `{` — only chars that can lead to a valid attribute/comment/empty body (alnum, `_`/`-`, `.`/`#`/`%`/`}`/`\\`/`\n`, whitespace) get to walk the body. Within noise; protects against the `{plain prose...}` walk-then-fail path.
- [~] **#10** Tried lowering `prec.dynamic` → `prec` for `_image`/`_link`. **Reverted.** Breaks "Link: empty text inline" (`[](y)`): the LALR generator picks the no-link branch when given a static precedence, while at runtime dynamic precedence + the scanner gates resolve it to a link. The TODO note assumed the scanner-side gates were sufficient to prune invalid branches, but with empty `[]` the bracketed path is *valid* — the conflict is between two valid parses and dynamic precedence is genuinely needed for tie-break. Leaving as `prec.dynamic`; no further action.
